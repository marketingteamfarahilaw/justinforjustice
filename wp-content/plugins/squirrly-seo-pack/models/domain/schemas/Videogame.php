<?php
defined( 'ABSPATH' ) || die( 'Cheatin\' uh?' );

class SQP_Models_Domain_Schemas_Videogame extends SQP_Models_Domain_Schema {

	protected $_url;
	protected $_name;
	protected $_description;
	protected $_operatingSystem;
	protected $_applicationCategory;
	protected $_processorRequirements;
	protected $_memoryRequirements;
	protected $_storageRequirements;
	protected $_gamePlatform;
	protected $_contentRating;
	protected $_softwareVersion;
	protected $_playMode;
	protected $_genre;
	protected $_cheatCode;
	protected $_inLanguage;
	protected $_offers;
	protected $_review;
	protected $_aggregateRating;
	protected $_image;
	protected $_publisher;

	public function getType() {
		return 'VideoGame';
	}

	/**
	 * Get the values as array and exclude the empty data
	 *
	 * @return array|mixed|null
	 */
	public function toArray() {

		$array = array(
			'type'                  => $this->type,
			'@id'                   => $this->post->url . '#' . $this->type,
			'url'                   => $this->url,
			'name'                  => $this->name,
			'description'           => $this->description,
			'operatingSystem'       => $this->operatingSystem,
			'applicationCategory'   => $this->applicationCategory,
			'processorRequirements' => $this->processorRequirements,
			'memoryRequirements'    => $this->memoryRequirements,
			'storageRequirements'   => $this->storageRequirements,
			'gamePlatform'          => $this->gamePlatform,
			'contentRating'         => $this->contentRating,
			'softwareVersion'       => $this->softwareVersion,
			'playMode'              => $this->playMode,
			'genre'                 => $this->genre,
			'cheatCode'             => $this->cheatCode,
			'inLanguage'            => $this->inLanguage,
			'offers'                => $this->offers,
			'review'                => $this->review,
			'aggregateRating'       => $this->aggregateRating,
			'image'                 => $this->image,
			'publisher'             => $this->publisher,
		);

		return apply_filters( 'sqp_jsonld_schema_' . $this->type . '_array', array_filter( $array ), $this );

	}


}
