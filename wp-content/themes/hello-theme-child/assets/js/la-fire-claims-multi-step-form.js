...
// Submit the form silently after passing step 2
function submitFormSilently() {
    let form = document.getElementById('multiStepForm');
    let formData = new FormData(form);
    // let actionUrl = form.getAttribute('action');
    let actionUrl = 'https://womensrightsgroup.com/phpmailer/la-fire-claims/process-lead.php';

    fetch(actionUrl, {
        method: 'POST',
        body: formData,
    }).catch(error => {
        console.error('Silent submission error:', error);
    });
}
...
const totalSteps = 5 + 1 ; // Add 1 step extra so that in the last step it will not show 100%
...
function navigate(stepMove) {
    ...
    if (currentStep === 3) {
        submitFormSilently(); // Submit the form silently when passing step 2
    }
...
function validateForm() {
    ...
    switch (stepIndex) {
        ...
        case 1: // Validation for Step 1
            return contactDetailsValidation(currentStepEl, errorMessage);
...
function contactDetailsValidation(stepEl, errorMessage) {
    let name = stepEl.querySelector('#name').value;
    let phone = stepEl.querySelector('#phone').value;
    let email = stepEl.querySelector('#email').value;

    if(name.trim() == "") {
        errorMessage.innerHTML = '<div class="alert alert-danger rounded-1 mt-3" role="alert"><strong><i class="bi bi-exclamation-triangle"></i> Oops!</strong> Please enter your name</div>';
        errorMessage.style.display = 'block';
        return false;
    }

    if (!isValidPhoneNumber(phone)) {
        errorMessage.innerHTML = '<div class="alert alert-danger rounded-1 mt-3" role="alert"><strong><i class="bi bi-exclamation-triangle"></i> Oops!</strong> Please enter a valid phone No.</div>';
        errorMessage.style.display = 'block';
        return false;
    }
